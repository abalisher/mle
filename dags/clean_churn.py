...
def clean_churn_dataset():
    ...
    def transform(data: pd.DataFrame):
        # Удаление дубликатов по признакам, кроме customer_id
        feature_cols = [col for col in data.columns if col != 'customer_id']
        data = data[~data.duplicated(subset=feature_cols, keep=False)]
        ...
        # Удаление выбросов по IQR для float колонок
